...
public class RecipeDAO {
	...
	/*
		  ...
		  <select id="recipeData7" resultType="RecipeVO">
		    SELECT no, title, poster, chef, hit, likecount, replycount
	...
	*/
	...
	/*
	  ...
	  /*
	    1. MyBatis
	        DML => selelct, update, delete, insert, sql
	                 |
	               resultMap : JOIN / SUQUERY
	        동적 쿼리
	          => <trim> : 추가 / 제거
	          => <bind> : 변수형 = 문장이 긴 경우
	              <bind name="likes" value="'%'||#{ss}||'%'">
	                #{likes}
	          => <foreach> : in 연산자데이터 여러개(checkbox)
