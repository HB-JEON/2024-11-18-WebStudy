...
public class QnABoardDAO {
	...
	/*
	  ...
	  <!-- 문의 -->
	  <select id="qnaInsert" parameterType="QnABoardVO">
	...
	/*
	 <select id="qnaAdminListData" resultType="QnABoardVO" parameterType="hashmap">
	    ...
	    FROM qnaBoard WHERE group_step=0 ORDER BY group_)id DESC))
	    WHERE num BETWEEN #{start} AND #{end}
	 </select>
	...
	/*
	 ...
	 <insert id="qnaAdminInsert" parameterType="QnABoardVO">
	    ...
	    VALUES((SELECT NVL(MAX(no)+1,1) 
	    FROM qnaBoard),#{id},'관리자',#{subject},#{content},#{pwd},#{group_id},1,1,'y'
	 </insert>
	 */
	...
	{
		SqlSession session=ssf.openSession();
		session.update("qnaDetailData", no);
