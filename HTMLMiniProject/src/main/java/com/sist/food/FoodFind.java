...
/*
   Servlet
     | 
   init() => 변수에 초기화
              web.xml 파일 읽기, 자동 로그인
     |
   사용자 요청 시 호출
     |
   doGet
   doPost
   ------ + service() => _jspService() => JSP 파일은 사용자에서 전송하는 파일
     |
   메모리 해제
   destroy() => 화면 이동 / 브라우저 종료 / 새로고침 => _jspDestroy()
   request / response / session / cookie
   ------------------------------ ------
    | 톰캣                          | 브라우저
   
   request / response : 지역변수 => 한번 사용하면 초기화
   session / cookie : 전역변수 => 기간 만료, 브라우저 종료, 로그아웃
     => 모든 서블릿 파일에서 사용 가능
     
   JSP = servlet 파일 변경 = 컴파일 = 실행 = 브라우저에서 HTML 열기
...
public class FoodFind extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<option value=type>음식 종류</option>");
